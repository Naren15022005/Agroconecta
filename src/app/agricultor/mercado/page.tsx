...
export default function MercadoPage() {
  ...
  // Mostrar banner con una transición ligera al montar
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-white overflow-x-hidden">
      ...
      {/* Banner grande de mercado (transición suave + overlay título) */}
      <section className={`w-full relative transform transition-all duration-700 ${bannerVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        ...
        {/* Overlay oscuro que cubre toda la imagen (mismo tono que el fondo del título) */}
        <div className={`absolute inset-0 transition-opacity duration-700 ${bannerVisible ? 'opacity-100' : 'opacity-0'}`}>
          <div className="absolute inset-0 bg-black/40" />
        </div>

        {/* Overlay con título centrado (fondo transparente ahora que la imagen tiene overlay) */}
      ...
      {/* (Categorías y búsqueda movidas dentro de 'Productos disponibles') */}

      {/* Contenido principal */}
      <main className="max-w-8xl mx-auto px-4 md:px-6 py-6">
        {/* Botón móvil: cuadro encima de las cards que abre el sidebar de filtros (oculto mientras cargan los productos) */}
        {productosLoaded && (
        ...
        {/* Panel lateral para móvil (overlay) */}
        ...
        {/* Layout principal: aside izquierdo con filtros + contenido de productos a la derecha */}
        <div className="mb-6">
